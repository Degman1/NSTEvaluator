...
class Evaluator:
    ...
    def artFidHandler(style_images_path, content_images_path, stylized_images_path):
        ...
        results = Evaluator.artFidEveryCombination(style_images_path, content_images_path, stylized_images_path)
    ...
    def artFidEveryCombination(style_images_path, content_images_path, stylized_images_path, cuda_device='0'):
        """
    ...
    def artFidSinglePairing(style_images_path, content_images_path, stylized_images_path, cuda_device='0'):
        """
        Calls the art_fid module to compute FID metric
    ...
    def structuralSimilarityEveryCombination (content_folder_path, stylized_folder_path):
        """
        ...
        temp_content_dir = "temp_content_folder"
        prepare_directory(stylized_folder_path, temp_stylized_dir)
        prepare_directory(content_folder_path, temp_content_dir)
        stylized_folder_path = temp_stylized_dir
        content_folder_path = temp_content_dir
        ssim_scores = []
    ...
    def structuralSimilaritySingularPairing (content_folder_path, stylized_folder_path):
        """
        Compute the structural similarity between the content image and the stylized image
        ...
        for i in range(len(content_files)):
            content_image_path = os.path.join(content_folder_path, content_files[i])
            stylized_image_path = os.path.join(stylized_folder_path, stylized_files[i])
            content_image = cv2.imread(content_image_path, cv2.IMREAD_GRAYSCALE)       #load the respective images
            stylized_image = cv2.imread(stylized_image_path, cv2.IMREAD_GRAYSCALE)
            if content_image is None or stylized_image is None:
                raise ValueError("One of the images could not be loaded.")              #error if one of the images isn't loaded properly
    ...
    def colorSimilarityEveryCombination (style_folder_path, stylized_folder_path) :
        """
        ...
        """
        temp_stylized_dir = "temp_stylized_folder"
        temp_style_dir = "temp_style_folder"
        prepare_directory(stylized_folder_path, temp_stylized_dir)
        prepare_directory(style_folder_path, temp_style_dir)
        stylized_folder_path = temp_stylized_dir
        style_folder_path = temp_style_dir
    ...
    def colorSimilaritySinglePairing (style_folder_path, stylized_folder_path) :
        """
        Compute the structural similarity between the style image and the stylized image
        ...
        for i in range(len(style_files)):
            style_image_path = os.path.join(style_folder_path, style_files[i])
            stylized_image_path = os.path.join(stylized_folder_path, stylized_files[i])
    ...
    def contentStyleLossEveryCombination(content_folder_path, style_folder_path, stylized_folder_path):
        """
    ...
    def contentStyleLossSinglePairing(content_folder_path, style_folder_path, stylized_folder_path):
        """
        Compute the combined content and style loss between the given images.
        ...
        content_files = sorted(os.listdir(content_folder_path))
        style_files = sorted(os.listdir(style_folder_path))
        for i in range(len(stylized_files)):
            content_file = content_files[i]
            style_file = style_files[i]
            stylized_file = stylized_files[i]
            if style_file == ".DS_Store" or content_file == ".DS_Store":
                continue
            content_image_path = os.path.join(content_folder_path, content_file)
            style_image_path = os.path.join(style_folder_path, style_file)
        ...
        try:
            ...
            for key, value in data.items():
                if key not in ["load_time", "unit"]:
                    ...
                    unit = value
            average_time = np.mean(times)  # Compute the average time
            std_dev_time = np.std(times)
            return average_time, std_dev_time, load_time, unit
    ...
    def animationPerformance(ssim_score, color_score, content_loss, style_loss, avg_time, load_time):
        ...
        score = alpha * ssim_score + beta * color_score + gamma * norm_content_loss + delta * norm_style_loss + epsilon * (1 / avg_time) + zeta * (1 / load_time)
        return score
        
    def evaluateEveryCombination(style, content, output):
        results = {}
        for model_dir in os.listdir(output):
            ...
            print("artfid done")
            ssims = Evaluator.structuralSimilarityEveryCombination(content, stylized_folder_path)
            print("ssims done")
            colorsims = Evaluator.colorSimilarityEveryCombination(style, stylized_folder_path)
            print("colors done")
            contentLoss, styleLoss = Evaluator.contentStyleLossEveryCombination(content, style, stylized_folder_path)
            print("loss done")
            avg_times, std_dev_times, load_time, unit = Evaluator.timePerformance(stylized_folder_path)
            print("times done")
            ...
            "ColorSim": (np.mean(colorsims), np.std(colorsims)),
            "ContentLoss": (np.mean(contentLoss), np.std(contentLoss)),
            "StyleLoss": (np.mean(styleLoss), np.std(styleLoss)),
            "AnimationScore": ani_score,
            "AvgTime": (avg_times, std_dev_times),
            "LoadTime": load_time
            }
            print(model_dir, " done")
        return results
    
    def evaluateSinglePairing(style, content, output):
        results = {}
        for model_dir in os.listdir(output):
            if model_dir == ".DS_Store":
                continue
            stylized_folder_path = os.path.join(output, model_dir)
            print(model_dir, "started")
            artfid = Evaluator.artFidSinglePairing(style, content, stylized_folder_path)
            print("artfid done")
            ssims = Evaluator.structuralSimilaritySingularPairing(content, stylized_folder_path)
            print("ssims done")
            colorsims = Evaluator.colorSimilaritySinglePairing(style, stylized_folder_path)
            print("colors done")
            contentLoss, styleLoss = Evaluator.contentStyleLossSinglePairing(content, style, stylized_folder_path)
            print("loss done")
            avg_times, std_dev_times, load_time, unit = Evaluator.timePerformance(stylized_folder_path)
            print("times done")
            ani_score = Evaluator.animationPerformance(np.mean(ssims), np.mean(colorsims), np.mean(contentLoss), np.mean(styleLoss), avg_times, load_time)
...
ai_content = "/Users/bmhall17/Downloads/results/output_ai/preprocessed_content"
ai_style = "/Users/bmhall17/Downloads/results/output_ai/preprocessed_style"
ai_output = "/Users/bmhall17/Downloads/results/output_ai/output"

comic_content = "/Users/bmhall17/Downloads/results/output_comic/preprocessed_content"
comic_style = "/Users/bmhall17/Downloads/results/output_comic/preprocessed_style"
comic_output = "/Users/bmhall17/Downloads/results/output_comic/output"

ai_evaluation_results = Evaluator.evaluateSinglePairing(ai_style, ai_content, ai_output)
comic_evaluation_results = Evaluator.evaluateSinglePairing(comic_style, comic_content, comic_output)

emotion_content = "/Users/bmhall17/Downloads/results2/output_emotions/preprocessed_content"
emotion_style =  "/Users/bmhall17/Downloads/results2/output_emotions/preprocessed_style"
emotion_output = "/Users/bmhall17/Downloads/results2/output_emotions/output"

frames_content = "/Users/bmhall17/Downloads/results2/output_frames/preprocessed_content"
frames_style =  "/Users/bmhall17/Downloads/results2/output_frames/preprocessed_style"
frames_output = "/Users/bmhall17/Downloads/results2/output_frames/output"

emotion_evaluation_results = Evaluator.evaluateSinglePairing(emotion_style, emotion_content, emotion_output)
frames_evaluation_results = Evaluator.evaluateSinglePairing(frames_style, frames_content, frames_output)

print("AI Results:")
print(ai_evaluation_results)

print("Comic Results:")
print(comic_evaluation_results)

print("Emotion Results:")
print(emotion_evaluation_results)

print("Frames Results:")
print(frames_evaluation_results)
